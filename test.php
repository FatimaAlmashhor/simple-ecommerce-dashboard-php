...
<?php
            ...
            if($do == 'add'){
                ?>
                    <form class='form' action="category.php?do=add" method='post'>
                        <input class='input ' type='title' name='category' placholder='Enter Category' require/>
                ...
                ?>
                    <form class='form' action="category.php?do=edit&catid=<?php echo $_GET['catid']?>" method='post'>
                        <input class='input'  type='title' name='category' placholder='Enter new Category' value='<?php echo $_GET['catid']?>' require/>
                        <button class='btn' type='submit'>Submit </button>
                    </form>
                <?php 
                 if(isset($_POST['category'])){
            ...
            foreach($rows as $row)
            {
                ...
                echo "<a class='' href='category.php?do=delete&catid=". $row['category_id'] ."'>Delete</a> <span>   </span>";
                echo "<a class='' href='category.php?do=edit&catid=". $row['category_id'] ."'>Edit</a>";
                echo "</div>";
            }
            echo "</div>
             <a class='' href='category.php?do=add'>Add new</a>
            ";
